...
class Game:
    ...
    def get_initial_map_data(self):
        ...
        raw_map_data = self.get_initial_map_data() 
    
        # Use pad_map to format it correctly
        self.map = self.pad_map(raw_map_data)
...
def cache_tree_positions(self):
        # Safe guard: do nothing if map isn't set yet or is empty
    self.tree_positions = []
...
def find_tree_positions(self):
    ...
    for y, row in enumerate(self.map):
        ...
        # A clever tool to mathematically erase white boxes and pink lines!
                # A clever tool to mathematically erase white boxes, gray borders, and pink lines!
        def clean_sprite_sheet(sheet):
            for x in range(sheet.get_width()):
                for y in range(sheet.get_height()):
                    r, g, b, a = sheet.get_at((x, y))
                    
                    # 1. Erase White and Gray backgrounds (including dirty/darker grays)
                    if r > 100 and g > 100 and b > 100 and abs(r-g) < 30 and abs(g-b) < 30:
                        sheet.set_at((x, y), (0, 0, 0, 0))
                        
                    # 2. Erase Magenta/Pink grid lines (including blended purple edges!)
        ...
        try:
            ...
            # --- FEATURE: The Ultimate Crop (Shaving 50 pixels horizontally!) ---
        ...
        self.leaf_base_sprite = self.load_sprite_image(LEAF_SPRITE_PATH, scale=False, fallback="none") # pyright: ignore[reportUndefinedVariable]
        self.leaf_cache = {} 
        ...
        # Ensure we scan the map for trees FIRST
        self.cache_tree_positions() 
        
        self.leaves = []
        for _ in range(60):
            # Pick a random tree to start at
            if self.tree_positions:
                tx, ty = random.choice(self.tree_positions)
                start_x = tx + random.randint(-40, 40)
                start_y = ty - random.randint(0, 200)
            else:
            ...
            self.leaves.append({
                'x': start_x,
                'y': start_y,
                # ... (keep your existing leaf properties like 'z', 'sway_phase', etc.)
            })
        ...
        # --- FEATURE: Falling Leaves Particle System ---
        self.leaf_particles = []
        for _ in range(60): # 60 leaves blowing in the wind
    ...
    def extract_map_entities(self, raw_map):
        ...
        for y in range(len(raw_map)):
            for x in range(len(raw_map[y])):
                ...
                # --- FEATURE: Custom Boss Map Editor Spawner ---
                    ...
                    enemies.append({
                        ...
                        # Calculate Threat Level from Designer Stats!
    ...
    def go_to_next_level(self):
        ...
        if not self.in_interior:
            ...
            # --- FEATURE: Spawn the Elite Custom Boss! ---
            if os.path.exists("custom_boss.json"):
                try:
                    with open("custom_boss.json", "r") as f:
                        b_data = json.load(f)
    ...
    def load_all_wall_textures(self):
        ...
        # --- FEATURE: Cracked Wall Generation (Secret Rooms) ---
    ...
    def load_all_floor_textures(self):
        ...
        try: textures['SAND'] = pygame.transform.scale(pygame.image.load(FLOOR_SAND_PATH).convert(), (TILE_SIZE, TILE_SIZE)) # pyright: ignore[reportUndefinedVariable]
        except:
    ...
    def generate_parallax_clouds(self):
        clouds = []
        for layer in range(CLOUD_LAYER): # pyright: ignore[reportUndefinedVariable]
            depth = layer / (CLOUD_LAYER - 1) if CLOUD_LAYER > 1 else 1.0   # pyright: ignore[reportUndefinedVariable]
            for _ in range(4 + layer):
                clouds.append({'x': random.randint(0, WIDTH), 'y': random.randint(5, 80 + layer * 20), 'scale': 3.0 + (depth * 3.5), 'depth': depth, 'speed_mult': CLOUD_SPEED_MULTIPLIERS[layer], 'alpha': 255 - int(depth * 25)}) # pyright: ignore[reportUndefinedVariable]
        return clouds
    ...
    def update(self):
        ...
        self.update_leaves() # <--- ADD THIS LINE HERE!
        ...
        for proj in self.projectiles[:]:
            ...
            # --- FEATURE: Enemy projectiles hurt the player! ---
            ...
            if 0 <= gx < w and 0 <= gy < h:
                # --- FEATURE: Magic Overloads Force Fields! ---
        ...
        if not self.in_interior:
            px_tile = int(self.player_x / TILE_SIZE)
            py_tile = int(self.player_y / TILE_SIZE)
            for y in range(max(0, py_tile - 6), min(h, py_tile + 7)):
                for x in range(max(0, px_tile - 6), min(w, px_tile + 7)):
                    if self.map[y][x] == TileType.TREE.value:
                        if len(self.leaves) < 50 and random.random() < 0.02:  
                            self.leaves.append({
                                'x': x * TILE_SIZE + 32 + random.uniform(-30, 30),
                                'y': y * TILE_SIZE + 32 + random.uniform(-30, 30),
                                'z': random.uniform(-400, -150),  
            ...
            for leaf in self.leaves[:]:
                leaf['z'] += leaf['speed']
                leaf['x'] += self.global_wind * 1.5 + math.sin(leaf['sway_phase']) * 2.5
                leaf['y'] += self.global_wind * 1.5 + math.cos(leaf['sway_phase']) * 2.5
        ...
        for e in self.enemies:
            ...
            if dist < 250:
                ...
                # --- FEATURE: Boss shoots fireballs & ANIMATES! ---
                if e.get('is_boss'):
        ...
        for e in dead_enemies:
            # --- FEATURE: Boss Loot Drops! ---
            if e.get('is_boss'):
                self.collect_xp(500) 
                
                # Drop the Dungeon Exit Key, an Artifact, and a Potion
    ...
    def draw_weather(self):
        ...
        # --- FPS OPTIMIZATION: Calculate math outside the loop ---
        ...
        if 'sand' in self.weather_type and not self.in_interior:
            # Create the surface once and reuse it to save massive amounts of CPU memory!
    ...
    def update_leaves(self):
        # Only update if we are outside and have leaves
        if self.in_interior or not hasattr(self, 'leaves') or not self.leaves: 
            return
        ...
        for leaf in self.leaves:
            ...
            # --- FEATURE: When a leaf falls off the screen, reset to a tree canopy ---
            if leaf['y'] > 2000: # Adjust this number if your map is taller!
                if self.tree_positions:
                    # Snaps the leaf back to a random tree!
                    tx, ty = random.choice(self.tree_positions)
                    leaf['x'] = tx + random.randint(-40, 40)
                    leaf['y'] = ty - random.randint(50, 150)
                else:
                    # Fallback if no trees found
                    leaf['x'] = random.randint(0, WIDTH)
                    leaf['y'] = -50

    def draw_hud(self):
        # --- FEATURE: Premium Animated Gold/Black HUD Design ---
        ...
        # Cap the math so the bars never draw past 100% capacity
    ...
    def draw(self):
        if not hasattr(self, 'map'):
            print("Warning: Map not loaded yet!")
            return
        h, w = len(self.map), len(self.map[0])
        ...
        for sprite in self.sprites:
            ...
            if px > 0.5: 
                ...
                if sprite.get('tex') in self.tree_leafy_sprites or sprite.get('tex') == self.tree_dead_sprite:
                    ...
                    # Scale up the boss based on the designer file!
                ...
                if 0 < sprite_h < HEIGHT * 3:
                    ...
                    if de_x > 0 and ds_x < WIDTH:
                        ...
                        # --- NEW BOSS GLOW CODE STARTS HERE ---
                        if sprite.get('is_boss'):
                            # Boss glows in the dark and pulses!
                        ...
                        if sprite.get('is_enemy') and 'ref' in sprite:
                            e = sprite['ref']
                            # 1. Depth & Distance Check (hide behind walls & far away)
    ...
    def perform_melee_attack(self):
        ...
        if self.stamina >= 15:
            ...
            # --- FEATURE: Secret Cracked Walls Destroyed By Melee! ---
    ...
    def use_hotkey_action(self, slot):
        ...
        if slot["type"] == "melee":
            ...
            slot["cd"] = slot["max_cd"] 
            if len(self.projectiles) > 4: self.projectiles.pop(0)
            self.projectiles.append({"x": self.player_x, "y": self.player_y, "angle": self.player_angle, "speed": 12.0, "tex": self.ui_icons.get("fireball"), "dmg": self.magic_dmg})
            if self.sfx.get("fireball"): self.sfx["fireball"].play()
    ...
    def get_id_for_item_name(self, name):
        mapping = {
            ...
            "Mana Potion": TileType.ITEM_MANA_POTION.value,
    ...
    def run(self):
        while True:
                        ...
                        if self.hovered_interactable:
                            if self.hovered_interactable["type"] == "door":
                                ...
                                if item['id'] == TileType.ITEM_ARTIFACT.value: 
                                    ...
                                    elif item['id'] == TileType.ITEM_HEALTH_POTION.value: success = self.inventory.add_item("Health Potion", 1, "consumable", "Restores 30 HP.", health=30, mana=0, stamina=0)
                                    elif item['id'] == TileType.ITEM_FOOD.value: success = self.inventory.add_item("Mana Potion", 1, "consumable", "Restores 30 Mana.", health=0, mana=30, stamina=0)
                                    elif item['id'] == TileType.ITEM_STAMINA_POTION.value: success = self.inventory.add_item("Stamina Potion", 1, "consumable", "Restores 50 Stamina.", health=0, mana=0, stamina=50)
                        ...
                        if ab_idx is not None:
                            if self.drag_source[0] == "inv":
                                ...
                                if target_slot["type"] == "magic":
                                    ...
                                    cost = 20 if self.drag_item["name"] == "Spell: Heal" else 15 if self.drag_item["name"] == "Spell: Frost" else 0
                                    if self.drag_item["name"].startswith("Spell:"): act_type = "magic"
                                    
                                    icon = self.inventory.get_icon_for_item(self.drag_item)
                                    self.action_bar.set_slot(ab_idx, self.drag_item["name"], icon, act_type, 30, cost)
                                    if self.sfx.get("pickup"): self.sfx["pickup"].play()
                                    self.save_game_state()
                            ...
                            if self.drag_source[0] == "ab":
                                # Prevent innate spells from being deleted
                                if self.drag_item["type"] == "magic":
                                    self.consume_message = "Can't remove innate spells!"
                                    self.consume_message_timer = 60
                                    if self.sfx.get("error"): self.sfx["error"].play()
                                else:
                            ...
                            if self.drag_source[0] == "ab":
                                # Prevent dropping spells on the ground
                                if self.drag_item["type"] == "magic":
                                    self.consume_message = "Can't drop spells!"
                                    self.consume_message_timer = 60
                                    if self.sfx.get("error"): self.sfx["error"].play()
                                else:
                                    self.action_bar.clear_slot(self.drag_source[1])
            ...
            if not self.inventory.visible and not self.level_complete and not self.game_over:
                ...
                walkable = [TileType.EMPTY.value, TileType.TREE.value, TileType.STANDING_TORCH.value, TileType.DEAD_TREE.value, TileType.BUSH.value, TileType.ROCK.value, TileType.ITEM_UNLIT_TORCH.value, TileType.ITEM_STAFF.value, TileType.STAIRS.value, TileType.ITEM_STAMINA_POTION.value]
                pad = 12
                is_moving = False
                
                curr_x_idx = int(self.player_x / TILE_SIZE)
                curr_y_idx = int(self.player_y / TILE_SIZE)
                target_x_pos = int((self.player_x + move_x + pad) / TILE_SIZE)
                target_x_neg = int((self.player_x + move_x - pad) / TILE_SIZE)
                target_y_pos = int((self.player_y + move_y + pad) / TILE_SIZE)
                target_y_neg = int((self.player_y + move_y - pad) / TILE_SIZE)

                if move_x > 0:
                    if 0 <= target_x_pos < MAP_SIZE and 0 <= curr_y_idx < MAP_SIZE:
                        if self.map[curr_y_idx][target_x_pos] in walkable:
                            self.player_x += move_x; is_moving = True
                elif move_x < 0:
                    if 0 <= target_x_neg < MAP_SIZE and 0 <= curr_y_idx < MAP_SIZE:
                        if self.map[curr_y_idx][target_x_neg] in walkable:
                            self.player_x += move_x; is_moving = True
                            
                if move_y > 0:
                    if 0 <= target_y_pos < MAP_SIZE and 0 <= curr_x_idx < MAP_SIZE:
                        if self.map[target_y_pos][curr_x_idx] in walkable:
                            self.player_y += move_y; is_moving = True
                elif move_y < 0:
                    if 0 <= target_y_neg < MAP_SIZE and 0 <= curr_x_idx < MAP_SIZE:
                        if self.map[target_y_neg][curr_x_idx] in walkable:
                            self.player_y += move_y; is_moving = True
                
                if is_moving:
                    if SFX_WALK and not CH_WALK.get_busy(): 
                        CH_WALK.play(SFX_WALK)
